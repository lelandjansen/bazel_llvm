...
elif [[ -n \$(rlocation clang-llvm-$version-x86_64-linux-gnu-ubuntu-20.04/bin/$binary) ]]; then
  \$(rlocation clang-llvm-$version-x86_64-linux-gnu-ubuntu-20.04/bin/$binary) "\$@"
else
...
sh_binary(
    ...
    data = select({
        ...
        "//conditions:default": [
            "@clang-llvm-10.0.0-x86_64-linux-gnu-ubuntu-20.04//:$binary",
        ],
    }),
    deps = [
